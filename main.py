...
async def help(interaction:discord.Interaction):
    embed = discord.Embed(title=f"⚙️・**Panel help**", color=0x2F3136) 
    embed.add_field(name="*Find all my orders below* :", value=f"""
    \n`/help` - **See all orders**
`/activity` - **Modify robot activity**
`/ping` - **Display robot latency**
`/boost` - **Boost the servers of your choice**
`/stock` -  **View boost stock in real time**
`/restock` - **Add tokens** (https://paste.ee)
`/clearstock` - **Removes everything in your stockpile**""")
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/1232349154840281150/1233428510157312043/Capture_decran_2024-04-23_a_23.37.46.png?ex=662d0f50&is=662bbdd0&hm=46886a54146efc09e7f60397bc9436c37b7a4443834ef6e31b86429f5d93efbf&=&format=webp&quality=lossless&width=934&height=930")
...
async def stock(interaction:discord.Interaction):
    ...
    embed = discord.Embed(title=f"Boost Stock",
        description=f"""__**1 Month Stock**__
`📦` Tokens : `{one}`
`🚀` Boosts : `{one*2}`
